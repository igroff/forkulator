...
# outputDirectory, when forkulator only ran things synchronously it
# was really a temp dir, but since it can run things in the background
# it's no longer temporary, for backwards compatability we allow continued
# usage of the old tem vars as well.
config=
  outputDirectory: process.env.COMMAND_OUTPUT_DIR ||
    process.env.FORKULATOR_TEMP ||
    process.env.TEMP ||
...
handleRequest = (req,res) ->
  ...
  # path starts with a /, so we'll hack it of for this 
  pathParts = req.path.substring(1).split "/"
  log.debug "parts: #{pathParts}"
  # it'll start with /, so we need the second element
  if pathParts[0] is "background"
    # so we'll take our path without the first element 'background'
    pathParts.shift()
    context.commandPath = pathParts.join "/"
    context.commandFilePath = path.join(config.commandPath, context.commandPath)
    context.runInBackground = true

  log.debug "background run requested: #{context.runInBackground}"
  ...
  .then (context) ->
    if context.runInBackground
      # normally we'd just write the object, but since we've already set the content type
      # and using write to write an object results in another attempt to set the content type
      # it's a no go
      Promise.resolve(res.write(JSON.stringify({message: "queued"})))
    else
